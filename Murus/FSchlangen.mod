IMPLEMENTATION MODULE FSchlangen;
...
CONST
  ...
  BEGIN
    ...
    WITH X^ DO
      IF i = ein THEN
        RETURN TRUE
      ELSE (* i = aus *)
        RETURN NOT Schlangen.leer (Schlange)
      END
    END
  END C;
  ...
  BEGIN
    ...
    WITH X^ DO
      IF i = ein THEN
        Schlangen.einfuegen (Schlange, A)
      ELSE (* i = aus *)
        Schlangen.entfernen (Schlange, A)
      END
    END
  END B;


PROCEDURE initialisieren (VAR ferneSchlange: Objekte; n: CARDINAL;
                          Anbieter: Namen; Nr: CARDINAL);
BEGIN
  NEW (ferneSchlange);
  WITH ferneSchlange^ DO
    Schlangen.initialisieren (Schlange, n);
