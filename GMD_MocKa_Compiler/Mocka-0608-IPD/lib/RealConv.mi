...
IMPLEMENTATION MODULE RealConv;
  ...
  FROM LREAL	IMPORT LFLOAT, LTRUNC, ATOF;
  FROM SYSTEM   IMPORT ADDRESS, ADR;
  (* FROM InOut    IMPORT WriteLongReal, WriteLn, WriteString; XXX DEBUG *)
  ...
  BEGIN
    RETURN Str2LongReal(s, done);
  END Str2Real;
     
  PROCEDURE Str2Real_old(s: ARRAY OF CHAR; VAR done: BOOLEAN): REAL;
  ...
  PROCEDURE Str2LongReal(s: ARRAY OF CHAR; VAR done: BOOLEAN): LONGREAL;
    VAR
      res: LONGREAL;
  BEGIN
    res := ATOF(ADR(s), ADR(done)); 
    (* XXX DEBUG
    WriteString("RealConv.Str2LongReal(): ");
    WriteLongReal(res, 10, -1);
    WriteLn;
    *)
    RETURN res;
  END Str2LongReal;

  PROCEDURE Str2LongReal_old(s: ARRAY OF CHAR; VAR done: BOOLEAN): LONGREAL;
  (* Converts the string 's' to real 'x'.			*)
  (* s has to be of the form:					*)
  (* real   = num ['.' {digit}] ['E' num].			*)
  (* num    = ['+'|'-'] digit {digit}.				*)
