...
(* ++ jv ++ *) (* 89/04/17 *)
    ...
    BEGIN
      ...
      i := 0;
      WHILE (NameOfSourceFile [i] # '.') AND 
            (NameOfSourceFile [i] # 0C) DO
         INC (i);
      END;
   
      IF    NameOfSourceFile [i] = 0C
      THEN  ERROR (
      "source is contained in a file without a suffix (must be '.mi' or '.md')",
                   CurPos);
      RETURN;
      END;
...
(***  auskommentiert
      INC (i); (* skip '.' *)
      IF   CAP (NameOfSourceFile [i]) = 'M' 
      THEN IF   MustBeDefinitionFile
           THEN IF   CAP (NameOfSourceFile [i + 1]) # 'D'
                THEN ERROR (
      "DEFINITION MODULE must be contained in a file with suffix '.md'", CurPos)
                END;
           ELSE IF   CAP (NameOfSourceFile [i + 1]) # 'I'
                THEN ERROR (
 "[IMPLEMENTATION] MODULE must be contained in a file with suffix '.mi'",CurPos)
                END;
           END;
      ELSE ERROR (
"source is contained in a file with illegal suffix (must be '.mi' or '.md')",
                   CurPos);
     END; 
