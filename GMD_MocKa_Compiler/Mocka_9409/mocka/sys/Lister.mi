...
MODULE Lister;
   ...
   FROM ByteIO IMPORT
      ...
      OpenInput, OpenOutput, Close, EOF, Done;
   ...
   PROCEDURE ReadMsgs;
	 ...
	 WHILE (errtab[i].line > line) OR
	       ((errtab[i].line = line) AND (errtab[i].col > col))
	 DO
	    DEC(i);
	 END;
	 (* insert after pos 'i' *)
   ...
   PROCEDURE GenListing (VAR sourcefilename: ARRAY OF CHAR;
      ...
      VAR
         ...
         ch : CHAR; linecount, errcount: SHORTCARD;

      PROCEDURE CreateHeadLine;
	 VAR header: ARRAY [0..100] OF CHAR; i: SHORTCARD;
      BEGIN
         header := "@ LISTING"; i := 0;
         WHILE header[i] <> 0C DO PutByte(ListFile, header[i]); INC(i); END;
	 ...
	 PutByte(ListFile, '@'); markcount := 0; nextcol := 2;
	 n := errcount;
	 WHILE (n <= errtablast) AND (errtab[n].line = linecount) DO
	    WHILE nextcol < errtab[n].col DO
	       PutByte(ListFile, ' '); INC(nextcol);
	    END;
	    IF nextcol = errtab[n].col THEN
	       INC(markcount); PutByte(ListFile, "^"); INC(nextcol)
	    END;
	    INC(n);
	 END;

	 IF markcount > 0 THEN PutByte(ListFile, 12C); LineClosed := TRUE;
	 ELSE LineClosed := FALSE;
	 END;

	 (* create lines containing error messages *)
	 markindex := 0; nextcol := 2;
	 WHILE (errcount<=errtablast) AND (errtab[errcount].line=linecount) DO
	    WHILE nextcol < errtab[errcount].col DO INC(nextcol) END;
	    IF nextcol=errtab[errcount].col THEN
	       INC(markindex); INC(nextcol)
	    END;
	    IF LineClosed THEN PutByte(ListFile, '@') END;
	    ...
	    WHILE stringtab[k] <> 12C DO
	       PutByte(ListFile, stringtab[k]); INC(k);
	    END;
	    PutByte(ListFile, 12C); LineClosed := TRUE;
	    INC(errcount);
	 END;
   ...
   PROCEDURE Main;
      VAR argc: SHORTCARD; argv: ArgTable;
      VAR errorfilename,listfilename: ARRAY[0..MaxFilenameLength] OF CHAR;
   BEGIN
      GetArgs (argc, argv);
